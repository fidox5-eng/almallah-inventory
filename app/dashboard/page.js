    ...
    <div style={{ minHeight: "100vh", background: "#0b0b0b", color: "#fff" }}>
      <div style={{ padding: 28, maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 16 }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 24 }}>Inventory Dashboard (INV2)</h1>
        ...
        <div style={{ marginTop: 18, border: "1px solid #1f1f1f", background: "#101010", borderRadius: 16, padding: 16 }}>
          ...
          <form onSubmit={addItem} style={{ marginTop: 12, display: "grid", gap: 10, gridTemplateColumns: "repeat(12, minmax(0, 1fr))" }}>
            ...
            <div style={{ gridColumn: "span 3" }}>
              <div style={{ color: "#aaa", fontSize: 12, marginBottom: 6 }}>Item ID</div>
              <input value={form.item_sku} onChange={(e) => setForm((f) => ({ ...f, item_sku: e.target.value }))}
                style={{ width: "100%", padding: 10, borderRadius: 12, border: "1px solid #2a2a2a", background: "#0b0b0b", color: "#fff" }} />
            </div>
            ...
            <div style={{ gridColumn: "span 4" }}>
              ...
              <input value={form.serial_or_imei} onChange={(e) => setForm((f) => ({ ...f, serial_or_imei: e.target.value }))}
                style={{ width: "100%", padding: 10, borderRadius: 12, border: "1px solid #2a2a2a", background: "#0b0b0b", color: "#fff" }} />
            </div>
            ...
            <div style={{ gridColumn: "span 2" }}>
              ...
              <input type="number" step="0.01" min="0" value={form.sell} onChange={(e) => setForm((f) => ({ ...f, sell: e.target.value }))}
                style={{ width: "100%", padding: 10, borderRadius: 12, border: "1px solid #2a2a2a", background: "#0b0b0b", color: "#fff" }} />
            </div>

            <div style={{ gridColumn: "span 2", display: "flex", alignItems: "end" }}>
              <button type="submit"
                style={{ width: "100%", padding: 12, borderRadius: 12, border: "1px solid #2a2a2a", background: "#fff", color: "#000", fontWeight: 800, cursor: "pointer" }}>
                Add
              </button>
            </div>
          </form>
                  ...
                  {items.map((it) => {
                    ...
                    return (
                      <tr key={it.id} style={{ borderBottom: "1px solid #161616" }}>
                        ...
                        <td style={{ padding: 10 }}>${money(profitTotal)} <span style={{ color: "#777", fontSize: 12 }}>(per: ${money(profitOne)})</span></td>
                        <td style={{ padding: 10 }}>
